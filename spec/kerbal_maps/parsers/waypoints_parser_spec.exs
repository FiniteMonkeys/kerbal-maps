defmodule KerbalMaps.WaypointsParser.Spec do
  @moduledoc false

  use ESpec

  doctest KerbalMaps.WaypointsParser
  ...
  example_group "parsing a single waypoint" do
    ...
    let :initial_data, do: []
    let :parsed, do: KerbalMaps.WaypointsParser.parse_waypoint({:ok, initial_data(), "", shared.stream})
  ...
  example_group "parsing example data" do
    ...
    let :initial_data, do: []
    let :parsed, do: KerbalMaps.WaypointsParser.parse({:ok, initial_data(), shared.stream})
    let :status, do: parsed() |> elem(0)
    let :data, do: parsed() |> elem(1) |> List.flatten
    let :remainder, do: parsed() |> elem(2)
  ...
  example_group "stripping a comment" do
    ...
    let :initial_data, do: []
    let :parsed, do: KerbalMaps.WaypointsParser.strip_comment({:ok, initial_data(), "", shared.stream})
    let :status, do: parsed() |> elem(0)
